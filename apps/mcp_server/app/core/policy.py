...
from app.db.models import Tenant, TenantAssetPolicy
...
class PolicyService:
    ...
    def resolve_context_for_request(self, session: Session, request_context: WorkerRequestContext) -> OperationalContext:
        if request_context.execution_profile == ExecutionProfile.client_scoped:
            ...
            if request_context.operational_context is not None:
                ...
                self.assert_context_allowed(policy, request_context.operational_context)
    ...
    def assert_context_allowed(self, policy: TenantPolicySnapshot, operational_context: OperationalContext) -> None:
